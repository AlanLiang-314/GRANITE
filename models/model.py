...
class ParamSelectorPlus(torch.nn.Module):
    def __init__(self,public_model,sz,frac,sparse_grad,seed):
        super(ParamSelectorPlus,self).__init__()
        #self.seed=seed
        # Initialize RNG for reproducible index selection
        ...
        self.frac = frac
        self.sparse_grad = sparse_grad
        assert not self.sparse_grad, "Hardcoded False in extract (TorchScript). Update extract before enabling."

        # Helper: choose (max(sz, frac * param_size)) unique, sorted indices per parameter
        def select_indices(param: torch.nn.Parameter, select_all: bool = False, target_size: int = None) -> torch.Tensor:
            param_size = param.numel()
            if target_size is None:
                target_size = max(self.sz, int(round(param_size * self.frac)))
            
            if select_all:
                perm = torch.linspace(0, param_size - 1, steps=param_size, dtype=torch.long)
                return perm.to(param.device).sort().values
            else:
                perm = torch.randperm(param_size, generator=gen)
                return perm[:target_size].to(param.device).sort().values

        # Indices within each parameter tensor
        self.rps = [select_indices(p, select_all=False) for _, p in public_model.named_parameters()]
        self.num_par = sum(rp.numel() for rp in self.rps)
        print(f"ParamSelector: selected {self.num_par} parameters.")
        
        param_needed = self.num_par
        self.rps = [None for _ in self.rps]
        
        selected_modules = set()
        # # first pass: take all BN params
        # for idx, (n, p) in enumerate(public_model.named_parameters()):
        #     if 'bn' in n.lower():
        #         # Take all BN params
        #         self.rps[idx] = select_indices(p, select_all=True)
        #         param_needed -= p.numel()
                
        #         selected_modules.add(n)
                
        # for idx, (n, p) in enumerate(public_model.named_parameters()):
        #     if self.rps[idx] is None:
        #         if param_needed < p.numel():
        #             # Sample remaining params to fill budget
        #             self.rps[idx] = select_indices(p, select_all=False, target_size=param_needed)
        #             param_needed -= self.rps[idx].numel()
        ...
        for idx, (n, p) in enumerate(reversed((list(public_model.named_parameters())))):
            idx = len(self.rps) - 1 - idx
            if self.rps[idx] is None:
                if param_needed < p.numel():
                    # Sample remaining params to fill budget
                    self.rps[idx] = select_indices(p, select_all=False, target_size=param_needed)
        ...
        assert param_needed <= 0, f"Parameter budget not met: need {param_needed} more."
        self.selected_modules = selected_modules
        
        # Compute flattened offsets so we can concatenate into global index tensor
        numels = [p.numel() for p in public_model.parameters()]
        cumulative = torch.tensor(numels).cumsum(0)
        offsets = [0] + cumulative[:-1].tolist()

        # Offsets applied to each parameter's local indices
        self.rp_cat = torch.cat([rp + off for rp, off in zip(self.rps, offsets) if rp is not None], dim=0)

        # Logging / statistics (no grad)
        with torch.no_grad():
            used = 0
            total = 0
            print("used/total\t\tpercent\t\tname\t\tshape")
            for rp, (name, p) in zip(self.rps, public_model.named_parameters()):
                if rp is None:
                    u = 0
                else:
                    u = rp.numel()
                ...
                print(f"{u}/{t}\t\t{pct}%\t\t{name}\t\t{shape_str}")
                used += u
                total += t
            ...
            print("summary")
            print(f"{used}/{total}\t\t{round(100 * self.frac_tot, 1)}%")
            print()
